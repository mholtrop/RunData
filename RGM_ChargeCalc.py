...
def setup_rundata_structures(data_loc):
    ...
    data_loc.Current_Channel = "scalerS2b"  # "IPM2C21A"  # "scaler_calc1b"
    ...
    data_loc.min_event_count = 500000  # Runs with at least 200k events.
    data_loc.start_time = datetime(2021, 11, 10, 8, 0)  # Start of run.
    data_loc.end_time = datetime(2022, 2,  8, 10, 0) # datetime(2021, 12, 21, 8, 0)    # End of first run period.
...
def main(argv=None):
    ...
    if not args.nocache:
        data = RunData(cache_file="RGM_Charge.sqlite3", i_am_at_jlab=at_jlab)
    else:
    ...
    print("Fetching the data from {} to {}".format(data.start_time, data.end_time))

    #data.get_runs(data.start_time, data.end_time, data.min_event_count)
