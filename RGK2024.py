...
def main(argv=None):
    ...
    if not args.nocache:
        data = RunData(cache_file="RGK.sqlite3", i_am_at_jlab=at_jlab)
    else:
    ...
    if args.plot:
        ...
        if args.fcup:
            title = "<b>RGK 2024 Progress</b>  FCup"
        else:
            title = "<b>RGK 2024 Progress</b>  IPM2C21"
        fig.update_layout(
        ...
        if args.chart:
            charts.plot(fig, filename='RGK_edit', width=2048, height=900, auto_open=True)
        if args.live:
